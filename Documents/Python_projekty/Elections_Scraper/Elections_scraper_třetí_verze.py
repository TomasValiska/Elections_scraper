...
# Vrací link vložený uživatelem
def link_elections_scraper(link: str):
  return link


# Vrací jméno zvolené uživatelem
def name_elections_scraper(name_output_file: str):
...
def get_counties_numbers(numbers):
  counties_numbers = [[], [], [], [], [], [], [], [], [], [], [], [], [], []]
  for x, y in enumerate(numbers):
    if "1100" in y:
      ...
      x = 13
      counties_numbers[x].append(y)
  return counties_numbers


# Vyhodnotí správnost či nesprávnost linku.  
def link_assessment(counties_numbers, link):
...
def output_name_assessment(name_output_file):
  if ".xlsx" not in name_output_file:
    outcome = "rejected"
  else:
...
def preserver(result_1, result_2):
...
def position_assessment(name_output_file, link, counties_numbers):
  if link_assessment(counties_numbers, name_output_file) == "the link verified" and output_name_assessment(link) == "the name verified":
...
def printer(preservation, result_3, link):
...
# Vytvoří list obsahující editovaná html data ze zdrojových url
def html_editor(link_muni):
  supplier = link_muni
  divided_response = []
  for link_supply in supplier:
    # získání a úprava zdrojového kódu
    response = get(link_supply)
    divided_response_0 = bs(response.text, features="html.parser")
    divided_response.append(divided_response_0)
  return divided_response

# Získá kód obce do výsledné tabulky
def muni_code(source_codes):
  codes = []
  for text_html in source_codes:
    temporary = []
    divs = text_html.find("div", {"class":"tab_full_ps311"})  
    a = divs.find('a')
    href = a.get('href')
    if "ps311?" in href:
      parts = href.split('=')[-3].split("&XVYBER")
      temporary.append(parts[0])
    codes.append(temporary)
  return codes

# Získá jméno obce do výsledné tabulky
def muni_name(source_names, muni_codes):
  codes_names = muni_codes
  for index, text_html in enumerate(source_names):
    h3s = text_html.find_all("h3")
    if h3s[0].text.split(":")[1].strip() == "Hlavní město Praha":
      names = h3s[1].text.split(":")[1].strip()
    else:
      names = h3s[2].text.split(":")[1].strip()
    codes_names[index].append(names)
  return codes_names

# Získá počet registrovaných voličů do výsledné tabulky
def voters_reg(source_voters, codes_and_names):
  cod_nam_vot = codes_and_names
  for index, text_html in enumerate(source_voters):
    tds = text_html.find('td', {"headers":"sa2"})
    if "\xa0" in tds.text:
      voters = tds.text.split("\xa0")[0]+tds.text.split("\xa0")[1]
    else:
      voters = tds.text
    cod_nam_vot[index].append(voters)
  return(cod_nam_vot)

# Získá počet odevzdaných obálek
def votes(source_votes, c_n_v):
  c_n_v_v = c_n_v
  for index, text_html in enumerate(source_votes):
    tds2 = text_html.find('td', {"headers":"sa5"})
    if "\xa0" in tds2.text:
      votes = tds2.text.split("\xa0")[0]+tds2.text.split("\xa0")[1]
    else:
      votes = tds2.text
    c_n_v_v[index].append(votes)
  return(c_n_v_v)


# Získá počet platných hlasů
def valid_votes(source_valid_votes, c_n_v_v):
  c_n_v_v_vv = c_n_v_v
  for index, text_html in enumerate(source_valid_votes):
    tds3 = text_html.find('td', {"headers":"sa6"})
    if "\xa0" in tds3.text:
      valid_votes = tds3.text.split("\xa0")[0]+tds3.text.split("\xa0")[1]
    else:
      valid_votes = tds3.text
    c_n_v_v_vv[index].append(valid_votes)
  return(c_n_v_v_vv)


# Získá počet platných hlasů pro jednotlivé strany
def party_votes(source_party_votes, c_n_v_v_vv):
  c_n_v_v_vv_pv = c_n_v_v_vv
  for index, text_html in enumerate(source_party_votes):
    tds4 = text_html.find_all('td', {"headers":"t1sa2 t1sb3"})
    for votes in tds4:
      if votes.text != "-" and "\xa0" in votes.text:
        party_votes = votes.text.split("\xa0")[0]+votes.text.split("\xa0")[1]
      elif votes.text != "-":
        party_votes = votes.text
      c_n_v_v_vv_pv[index].append(party_votes)
    tds5 = text_html.find_all('td', {"headers":"t2sa2 t2sb3"})
    for votes_2 in tds5:
      if votes_2.text != "-" and "\xa0" in votes_2.text:
        party_votes = votes_2.text.split("\xa0")[0]+votes_2.text.split("\xa0")[1]
      elif votes_2.text != "-": 
        party_votes = votes_2.text
      else:
        party_votes = None
      c_n_v_v_vv_pv[index].append(party_votes)
  rows = c_n_v_v_vv_pv
...
def xlsx_maker(list_of_headers, rows, name_output_file):
...
if __name__ == "__main__":
  ...
  # Vyvtoří scénář s náhodně generovanými správnými vstupy, scenář s chybnými vstupy a scénář
  ...
  # Slovník chybných vstupů, správných vstupů a správných vstupů na chybných pozicích. 
  ...
  # Postupné volání funkcí a ukládání jejich výstupů do proměnných
  link = link_elections_scraper(generated_link)
  file_name = name_elections_scraper(generated_name_file)
  numbers = get_numbers()
  counties_numbers = get_counties_numbers(numbers)
  result_1 = link_assessment(counties_numbers, link)
  result_2 = output_name_assessment(file_name)
  preservation = preserver(result_1, result_2)
  result_3 = position_assessment(file_name, link, counties_numbers)
  printer(preservation, result_3, link)
  links = generating_links(link)
  headers = headers_maker(links)
  source_codes = html_editor(links)
  codes = muni_code(source_codes)
  names = muni_name(source_codes, codes)
  voters = voters_reg(source_codes, names)
  votes_data = votes(source_codes, voters)
  valid_votes_data = valid_votes(source_codes, votes_data)
  final_data = party_votes(source_codes, valid_votes_data)
  xlsx_maker(headers, final_data, file_name)
